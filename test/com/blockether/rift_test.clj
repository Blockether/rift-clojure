...
(deftest roundtrip
  (testing "real rift: init -> create -> clean -> list -> ancestors -> excluded -> remove -> gc"
    (let [src (temp-dir "rift-src")
      ...
      (let [commit (str/trim (:out (sh/sh "git" "rev-parse" "HEAD" :dir src)))]
        (spit (io/file src "README.md") "staged intermediate\n")
        (sh/sh "git" "add" "README.md" :dir src)
        (spit (io/file src "untracked.txt") "untracked\n")
        ...
        (let [ws (rift/create {:from (str src) :name "feature" :database (str db)})]
          ...
          (is (= "staged intermediate\n" (slurp (io/file ws "README.md")))
              "ordinary create retains working-tree state")
          (is (seq (:out (sh/sh "git" "status" "--porcelain" :dir ws)))
              "ordinary create retains the copied dirty index and worktree")

          ;; Regression: clean used to rewrite bytes through a temporary index but
          ;; leave the workspace's real copied index dirty.
          (is (= ws (rift/clean! {:at ws :commit commit :database (str db)})))
          (is (= "" (:out (sh/sh "git" "status" "--porcelain" :dir ws)))
              "clean resets the real index and worktree")
          (is (= commit (str/trim (:out (sh/sh "git" "rev-parse" "HEAD" :dir ws))))
              "clean detaches at the exact requested commit")
          (is (= "hello rift\n" (slurp (io/file ws "README.md"))))
          (is (not (.exists (io/file ws "untracked.txt"))))
          (is (= "staged intermediate\n" (slurp (io/file src "README.md")))
              "clean never mutates the source")
          (is (.exists (io/file src "untracked.txt")) "source untracked state remains")
          ...
          (is (some #{"untracked.txt"} (rift/excluded {:of ws :database (str db)}))
              "excluded reports paths removed by clean")
...
;; Regression: clean refused every workspace whose source was not a Git
;; repository, so a project that was never `git init`-ed could not be cleaned.
(deftest clean-without-git
  (testing "real rift: clean leaves a non-Git workspace exactly as it was copied"
    (let [src (temp-dir "rift-plain")
          db  (io/file (temp-dir "rift-plain-reg") "registry.sqlite")]
      (spit (io/file src "README.md") "hello rift\n")
      (rift/init {:at (str src) :database (str db)})
      (let [ws (rift/create {:from (str src) :name "plain" :database (str db)})]
        (is (= ws (rift/clean! {:at ws :database (str db)}))
            "clean succeeds where there is no committed state to restore to")
        (is (= "hello rift\n" (slurp (io/file ws "README.md")))
            "and changes nothing in the workspace")
        (is (empty? (rift/excluded {:of ws :database (str db)}))
            "recording no exclusions")
        (rift/remove! {:at ws :database (str db)})))))
